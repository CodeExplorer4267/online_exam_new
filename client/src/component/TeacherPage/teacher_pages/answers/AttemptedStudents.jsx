  ...
  useEffect(() => {
    const fetchStudents = async () => {
      const res = await axios.get(
        `http://localhost:5000/online-exam/results/${examId}`
      );
        ...
        <table className="w-full text-sm text-gray-300">
          ...
          <tbody>
            {students.map((stu, idx) => (
              <tr
                key={idx}
                className="border-b border-gray-700 hover:bg-cyan-500/10 transition"
              >
                ...
                <td className="px-6 py-4">
                  <button
                    onClick={() =>
                      navigate(`/teacher/result/${examId}/${stu.studentId}`)
                    }
