...
const EachStudentAnswer = () => {
    ...
    const handleMarksForStudent=(e,index,totalMarks_for_each)=>{
       ...
       const total = updatedAnswers.reduce((sum, ans) => sum + (ans.marksGiven || 0), 0);
       setTotalMarks(total);
    }

    const handleSubmitMarks=()=>{
        axios.post(`http://localhost:5000/online-exam/update-marks`,{
            studentId,
            examId,
            marks:totalMarks
        })
  ...
  return (
    <>
      <div className="each_stu_ans_container">
         <h2 className='each_stu_ans_container_heading'>Answers of student : {student_name}</h2>
                ...
                {answers.map((a,index)=>{
                    return(
                        <tr key={index}>
                          ...
                          <td>{a.answer}</td>
                          <td>{a.marks}</td>
                          <td><input type="text" onChange={(e)=>handleMarksForStudent(e,index,a.marks)} disabled={a.isSubmitted}/></td>
                          <td>{a.isSubmitted ? <button>Marks already Submitted</button> : <button>Marks not Submitted</button>}</td>          
                        </tr>
                    )
                })}
            </tbody>
         ...
         <div className="marks_for_each_student">
               <p style={{
                fontSize:"20px",
                fontWeight:"bold",
               }}>Total marks for the Exam: {totalMarks} out of {examTotalMarks}</p>
         </div>
         <div className="submit_marks_btn">
            <button style={{
                ...
                cursor:"pointer",
                borderRadius:"10px",
            }} onClick={handleSubmitMarks}>Submit</button>
         </div>
      </div>
    </>
  )
